<!--- Must be CFINCLUDEd from the webroot Application.cfc --->
...
<cffunction name="setup">
	...
	<cfif request.coyote_setup_run eq 1>
		...
		<cfset local.settings.coyote_path = coyote_path>
		<cfset local.settings.coyote_mapping = coyote_mapping>
		<cfset local.settings.coyote_root = ExpandPath("#coyote_path#")>
		<cfif local.debug><cfoutput><p>PLe: #now()#</p></cfoutput></cfif>

		<!--- Config paths for COYOTE Settings and Components --->
		<cfset local.settings.coyote_config_path = 					"#local.settings.coyote_mapping#/coyote/config">
		...
		<cfset local.settings.glacier_config_path = 				"#local.settings.coyote_mapping#/glacier/config">
		...
		<cfset local.settings.application_config_path = 			"#local.settings.coyote_mapping#/#application.ApplicationName#/config">
		...
		<cfloop list="coyote_config_path_settings,glacier_config_path_settings,application_config_path_settings" index="local.key">
			...
			<cfif FileExists(local.path)>
				...
				<cfset local.tmp_settings = ensureIsArray(xmlFileToStruct(local.path).settings.setting)>
		...
		<cfloop list="coyote_config_path_components,application_config_path_components" index="local.key">
			...
			<cfif FileExists(local.path)>
				<cfset local.tmp_components = ensureIsArray(xmlFileToStruct(local.path).components.family)>
...
<cffunction name="addGlobalFunctions">
	...
	<cfset structAppend(url, createObject("component", "model.globalFunctions"))/>
</cffunction>
