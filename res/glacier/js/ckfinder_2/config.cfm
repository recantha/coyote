...
<cfset config = {}>
<cffunction name="checkAuthentication" returntype="boolean" access="public">
	<cfset var local = {}>

	<cfset local.auth = false>
	<cftry>
		<cfset local.sess = application.sessions[session.sessionid]>

		<cfquery datasource="#local.sess.ds#" username="#local.sess.un#" password="#local.sess.pw#" name="local.session">
			SELECT *
			FROM sessions
			WHERE session_id = <cfqueryparam cfsqltype="cf_sql_char" value="#local.sess.si#">
		</cfquery>

		<cfif local.session.recordCount>
			<cfquery datasource="#local.sess.ds#" username="#local.sess.un#" password="#local.sess.pw#" name="local.user">
				SELECT *
				FROM users
				WHERE id = <cfqueryparam cfsqltype="cf_sql_integer" value="#local.session.user_id#">
			</cfquery>

			<cfif local.user.recordCount>
				<cfset local.auth = true>
			</cfif>
		</cfif>
